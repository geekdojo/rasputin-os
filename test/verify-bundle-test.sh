...
mkrootfs "$W/n100-codesign.img" "$N100_CONF" "check-purpose=codesign"
...
expect refuse "codeSigning leaf without emailProtection is refused under today's device config (no check-purpose = smimesign)" \
	"$W/n100-nosmime.raucb" "$W/n100.img" "unsuitable certificate purpose"
# Same bundle, a device config that says check-purpose=codesign: now accepted.
# This is the case that shows the verify step applies the config it reads out
# of the image rather than RAUC's built-in default.
expect ok "the same leaf is accepted when the image's config sets check-purpose=codesign" \
	"$W/n100-nosmime.raucb" "$W/n100-codesign.img" "check-purpose=codesign"
